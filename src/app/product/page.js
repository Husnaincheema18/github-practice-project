...
useEffect(()=>{
  ...
  return (
    <div className="mt-20 m-auto w-4/5">
      <h1 className={`text-center ${styles.h1} text-gray-300`}>Product Data</h1>
      <div className=" grid lg:grid-cols-3 gap-5 mt-10">
        {
          ...
          <Link href={`/product/${currentItem.id}`}>
               ...
               <h1 className={`${styles.h1} text-center text-gray-400 mt-5`}>{currentItem.name}</h1>
               <p className="mt-5 text-center">{currentItem.description.slice(0, 130)}</p>
               <div className="text-center">
               <button className={`${styles.btn}`}>More detail</button>
               </div>
          </div>
        })}
      </div>
